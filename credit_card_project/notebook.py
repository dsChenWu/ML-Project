...
cc_apps_train, cc_apps_test = model_selection.train_test_split(cc_apps, test_size=0.33, random_state=42)
...
# cc_apps_train = cc_apps_train.replace('?', 'NaN', inplace = True)
# cc_apps_test = cc_apps_test.replace('?', 'Nan', inplace = True)
...
print('-'*100)
cc_apps_test = cc_apps_test.reindex(columns= cc_apps_train.columns, fill_value=0) #reindex in here is to double check the index between training data and testing data; using fill_value = 0 to replace the missing values as 0.
print(cc_apps_test)
